        ...
        {campaigns.map(c=> (
          <div key={c.id} style={{background:'#fff',borderRadius:12,padding:12,boxShadow:'0 6px 18px rgba(2,6,23,0.06)'}}>
            ...
            <div style={{display:'flex',gap:8}}>
              ...
              <Link to={c.type==='text'?`/text/${c.id}`:'/campaigns/'+c.id} className="btn btn-primary" style={{textDecoration:'none',display:'inline-flex',alignItems:'center',justifyContent:'center'}}>Open</Link>
            </div>
          </div>
        ))}
      </div>
